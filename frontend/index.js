import { FormField, Input, ViewPicker, initializeBlock,useGlobalConfig, useSettingsButton, 
    ...
    FieldPickerSynced,
	loadCSSFromString,
	colors} from '@airtable/blocks/ui';
...
const GlobalConfigKeys = {
	...
	MEMBER_LAST_VISIT_FIELD_ID: 'lastVisitFieldID',
...
function Payment() {
    ...
    const lastVisitFieldId 	= globalConfig.get(GlobalConfigKeys.MEMBER_LAST_VISIT_FIELD_ID);


    const initialSetupDone = memberTableId && paymentTableId && linkFieldId  &&
							 dateFieldId && nameFieldId && addressFieldId && postcodeFieldId && lastVisitFieldId ? true : false;
	...
	const lastVisitField = memberTable ? memberTable.getFieldByIdIfExists(lastVisitFieldId) : null;
				...
				<div>
					...
					{memberRecords.map(record => (
						<li key={record.id}>
							...
							<Text varient="span" textColor="red" display="inline-block" size="xlarge"> {record.getCellValue(lastVisitField)}</Text>
						</li>
					))}
...
function SettingsMenu(props) {
    ...
    const resetPaymentTableRelatedKeys = () => {
        ...
        props.globalConfig.setAsync(GlobalConfigKeys.MEMBER_LAST_VISIT_FIELD_ID, '');
				...
				{props.memberTable &&
                    <div>
                        ...
                        <Box display="flex" flexDirection="row">
                            ...
                            <FormField label="Lastvisit:" marginRight={1}>
                                <FieldPickerSynced
                                    size="small"
                                    table={props.memberTable}
                                    globalConfigKey={GlobalConfigKeys.MEMBER_LAST_VISIT_FIELD_ID}
                                    allowedTypes={[
                                        FieldType.DATE,
										FieldType.FORMULA
                                    ]}
